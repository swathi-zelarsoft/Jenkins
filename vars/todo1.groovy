...
def code_build(APP_TYPE, COMPONENT) {
    if(APP_TYPE == "NGINX") {
        command = "npm install --save-dev node-sass && npum audit fix && npm run build"
        def execute_com=sh(returnStdout: true, script: command)
        print execute_com
    }
    else if(APP_TYPE == "GO_LANG") {
...
def nexus(COMPONENT) {
    ...
    def get_branch_exec=sh(returnStdout: true, script: get_branch)
    def FILENAME=COMPONENT+'-'+get_branch_exec+'.zip'
    command = "curl -f -v -u admin:admin123 --upload-file /home/ubuntu/workspace/CI-Pipelines/${COMPONENT}-ci/${FILENAME} http://192.168.0.16:8081/repository/${COMPONENT}/${FILENAME}"
